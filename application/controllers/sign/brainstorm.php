		...
		if(empty($key)){
			...
			if($this->apikey($key)){
				...
				if(empty($schoolid) || empty($teacherid)){
					...
					$result=$this->brainstorm_model->insertInfo($data_p);
					$insert_id=$this->db->insert_id();
					if($insert_id){
						$data=$this->httpCode(200,'创建主题成功','');
					}else{
						$data=$this->httpCode(204,'创建主题失败','');
					}
				}
			}else{
	...
	public function getBsIist(){
		...
		if(empty($key)){
			...
			if($this->apikey($key)){
				...
				$get_topic=$this->db->select('id,content,addtime')->where("classid=$classid  and fid=0")->order_by("id", "desc")->get('brainstorm')->result_array();
	...
	public function  addBsContent(){
		...
		if(empty($key)){
			...
			if($this->apikey($key)){
				...
				//通过话题id获取字段信息
				$re=$this->db->select('schoolid,classid')->where("id=$topicid")->get('brainstorm')->row_array();
				...
				$insert_re=$this->db->insert('brainstorm',$data_p);
				$insert_id=$this->db->insert_id();
				if(empty($insert_id)){
	...
	public function  getBsContent(){
		...
		if(empty($key)){
			...
			if($this->apikey($key)){
				//接收参数
				$topicid=$this->input->post('topicid');
				$dis_content=$this->db->select('replyid,content,addtime')->where("fid=$topicid")->get('brainstorm')->result_array();
