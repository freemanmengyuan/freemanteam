...
class stuBrainstorm  extends base{
	...
	public function getBsList(){
		...
		if(empty($key)){
			...
			if($this->apikey($key)){
				...
				//获取讨论话题列表
				$get_topic=$this->db->select('id,content,addtime')->where("classid=$classid  and fid=0")->get('brainstorm')->result_array();
				if(empty($get_topic)){
					$data=$this->httpCode(204,'该课程尚未创建头脑风暴话题','');
				}else{
		...
		if(empty($key)){
			...
			if($this->apikey($key)){
				...
				if(empty($topicid) || empty($moreid)){
					...
					$offset=20*($moreid-1);
					$nums=20;
					$sql="select  id,replyid,content,addtime  from  brainstorm  where fid=$topicid  order by id  desc limit $offset,$nums";
					$bs_content=$this->db->query($sql)->result_array();
					if(empty($bs_content)){
						$data=$this->httpCode(204,'没有更多评论信息','');
					}else{
						foreach($bs_content as $k=>$v){
							$replyid=$v['replyid'];
							$re=$this->db->select('truename')->where("userid=$replyid")->get('user')->row_array();
							$truename=$re['truename'];
							$bs_content[$k]['truename']=$truename;
						}
						$data_re['userid']=$_SESSION['userid'];
						$data_re['data']=$bs_content;
