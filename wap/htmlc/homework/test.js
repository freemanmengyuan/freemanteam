...
(function () {
    ...
    var workid = window.location.search.substr(1);
    $.ajax({
        url:apiUrl+"myhomeworkcontent",//服务器请求地址
        type:'get',//提交方式 GET POST PUT DELETE
        data: {'userid':userid,'key':key,'schoolid':schoolid,'workid':workid},//传给服务器的值
        dataType:'json',
            ...
            if(data.code!="100"){
                ...
                if(test.type=="1"){
                    for(var j =0;j<test.options.length;j++){
                        ...
                        testmain += "<div class='testoption'><input type='checkbox' class='option-input radio' name='text'"+checkd+" value='"+test.options[j].code+"'><div class='optionmain'>"+test.options[j].code+":"+test.options[j].content+"<p></div>"
                    }
                }else{
...
function save(num) {
  if($('.save').hasClass('disabled')){
    ...
    var schoolid = localStorage.schoolid;
    var workid = window.location.search.substr(1);
    var test = {"userid":userid,"workid":workid,"isdone":num,"data":arr1}  /*   1是保存 2是提交  */
    //console.log(JSON.stringify(data))
    //console.log(arr1)
    $.ajax({
        url: apiUrl + "myhomeworksubmit",//服务器请求地址
